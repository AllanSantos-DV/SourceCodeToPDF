...
class MainWindow:
    ...
    def create_widgets(self):
        title = ttk.Label(self.root, text="Conversor de Código-Fonte para PDF", font=("Helvetica", 16, "bold"))
        title.pack(pady=10)

        select_btn = ttk.Button(self.root, text="Selecionar Pasta", command=self.select_folder, bootstyle=PRIMARY)
        select_btn.pack(pady=10)

        self.progress = ttk.Progressbar(self.root, mode='determinate', bootstyle=SUCCESS)
        self.progress.pack(pady=10)

        generate_btn = ttk.Button(self.root, text="Gerar PDF", command=self.generate_pdf, bootstyle=SUCCESS)
    ...
    def show_file_selection_window(self, files_with_states):
        ...
        for file, is_selected in files_with_states:
            ...
            file_vars[file] = var
            ttk.Checkbutton(scrollable_frame, text=file, variable=var, bootstyle="round-toggle").pack(anchor=W, padx=10, pady=2)
        ...
        confirm_btn = ttk.Button(selection_window, text="Confirmar Seleção", command=confirm_selection, bootstyle=SUCCESS)
